...
public final class ArrayUtils {
    ...
    public static int[] applyAll(int[] arr, int[] arr2, BinaryOperator<Integer> op) {
        ...
        int[] result = new int[longerArr.length];
        ...
        System.arraycopy(longerArr, shorterArr.length, result, shorterArr.length, longerArr.length - shorterArr.length);
    ...
    public static int[] addAll(int[] arr, int[] arr2) {
        return applyAll(arr, arr2, (a, b) -> a + b);
    }

    public static int[] addAll(int[] arr, int f) {
        return Arrays.stream(arr).map(a -> a + f).toArray();
    }
    ...
    public static double[] multAll(double[] arr, double f) {
        return Arrays.stream(arr).map(a -> a * f).toArray();
    }

    public static double[] divideAll(double[] arr, double f) {
        return Arrays.stream(arr).map(a -> a / f).toArray();
    }
